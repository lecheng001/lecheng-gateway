...
func NewJsonFromFile(file string) (*Json, error) {
	configcontent, _ := ioutil.ReadFile(file)
	return NewJson(configcontent)
}
...
func (j *Json) Set_bak(key string, val interface{}) {
	m, err := j.Map()
	if err != nil {
		return
	}
...
func (j *Json) Get(key string) *Json {
	//包含“.”字符，分隔成数组
	if strings.Contains(key, ".") {
		result := j.GetPath(strings.Split(key, ".")...)
		return result
	}
	...
	if err == nil {
		...
		_, err := j.Array()
		if err == nil {
			return j.GetArray(key)
		} else {
			_, err := j.MapArray()
			if err == nil {
				return j.GetMapArray(key)
			}
		}
	}
	return &Json{nil}
...
func (j *Json) GetMapArray(key string) *Json {
	m, err := j.MapArray()
	if err == nil {
		if val, ok := m[key]; ok {
			return &Json{val}
...
func (j *Json) GetArray(key string) *Json {
	m, err := j.MapArray()
	if err == nil {
		if val, ok := m[key]; ok {
			return &Json{val}
